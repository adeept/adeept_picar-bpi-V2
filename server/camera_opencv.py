...
class CVThread(threading.Thread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.elementDrawResult = 0
        self.findline_mark = 0
    ...
    def elementDraw(self,imgInput):
        # print(findLine_to_findColor_mark)
        if self.CVMode == 'none':
            pass

        elif self.CVMode == 'findColor' or findLine_to_findColor_mark == 2: 
            if self.findColorDetection:
                ...
                self.drawing = 0
                self.elementDrawResult = 0 # The camera does not recognize the set color.
            ...
            if self.radius > 10 and self.drawing:
                ...
                self.elementDrawResult = 1 # Indicates that the camera has recognized the set color.

        elif self.CVMode == 'findlineCV' or findLine_to_findColor_mark == 1:
    ...
    def findLine_to_findColor(self, frame_image):
        global lineColorSet, colorUpper, colorLower, findLine_to_findColor_mark
        dist = ultra.checkdist() # Read the distance value detected by the ultrasonic module. Unit: m
        # self.findline_mark == 0，It means that the video line tracking function will only be entered 
        # at the beginning. After exiting, the function cannot be entered again.
        if dist>=0.2 and self.findline_mark == 0:  # the distance is greater than 0.2m (20cm)
            # In the video line tracking stage
            
            # flask_app.camera.linePosSet_1(pos) # Video line patrol area settings, the position of the two lines
            # flask_app.camera.linePosSet_2(pos)
            findLine_to_findColor_mark = 1 # WEB display displays video line patrol content.
            self.findlineCV(frame_image)   # Raspberry Pi implements video line patrol function.
            lineColorSet = 0 # 255: Find white line, 0: Find black line.

        elif dist < 0.2:  # The distance is less than 20cm.
            # Start video color recognition.
            CVThread.scGear.moveAngle(1, 0) # The camera servo centered.
            move.motorStop() # Motor stops.

            # colorUpper,colorLower .The HSV maximum and minimum values for color recognition 
            # need to bemanually entered according to the recognized color. Values can be determined 
            # using the color recognition function.
            colorUpper = np.array([15, 255, 255])
            colorLower = np.array([0, 55, 105])
            findLine_to_findColor_mark = 2 # WEB interface displays color recognition content
            self.findColor(frame_image)    # The Raspberry Pi handles the color recognition stuff. (separate from WEB display)
            if self.elementDrawResult == 1: # Recognize the set color.
                print("Recognize the set color")
                self.findline_mark = 1 # The video line tracking phase ends.
                # What you need to do to add a car.
            else:
                pass
                findLine_to_findColor_mark = 0  # Display the original video without Raspberry Pi processing.
    ...
    def run(self):
        while 1:
            ...
            elif self.CVMode == 'findLine_to_findColor':
                self.CVThreading = 1
                self.findLine_to_findColor(self.imgCV)
                self.CVThreading = 0
            pass
